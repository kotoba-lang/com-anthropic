(ns anthropic.main
  "Kotodama WASM entrypoint for the Anthropic clean-room actor (L4) — Clojure port.
  ...
  facts (`anthropic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "anthropic")
(def tier "L4")
...
(def entity-specs
  [{:entity "Model"        :plural "models"        :id-prefix "anthropi_mod"
    :fields [:name :family :contextWindow :maxOutputTokens] :required [:name :family]
    :coerce {:contextWindow :int :maxOutputTokens :int} :refs {}}
   {:entity "Message"      :plural "messages"      :id-prefix "anthropi_mes"
    :fields [:modelId :role :content :inputTokens :outputTokens] :required [:role :content]
    :coerce {:inputTokens :int :outputTokens :int} :refs {:modelId "Model"}}
   {:entity "Conversation" :plural "conversations" :id-prefix "anthropi_con"
    :fields [:modelId :systemPrompt :turnCount] :required [:systemPrompt :turnCount]
    :coerce {:turnCount :int} :refs {:modelId "Model"}}
   {:entity "Tool"         :plural "tools"         :id-prefix "anthropi_too"
    :fields [:name :description :inputSchemaRef] :required [:name :description]
    :coerce {} :refs {}}
   {:entity "ToolUse"      :plural "tooluses"      :id-prefix "anthropi_too"
    :fields [:messageId :toolName :inputRef :status] :required [:toolName :status]
    :coerce {} :refs {:messageId "Message"}}
   {:entity "Batch"        :plural "batches"       :id-prefix "anthropi_bat"
    :fields [:modelId :requestCount :status] :required [:requestCount :status]
    :coerce {:requestCount :int} :refs {:modelId "Model"}}
   {:entity "File"         :plural "files"         :id-prefix "anthropi_fil"
    :fields [:filename :mimeType :bytes] :required [:filename :mimeType]
    :coerce {:bytes :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"anthropic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "anthropic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :anthropic-compat/ready)
